...
val_examples = tf.data.TFRecordDataset(valid_tfrecord).map(_parse_example)
# 读取字典
...
def encode(en_t, zh_t):
    ...
    en_bos = subword_encoder_en.vocab_size
    en_indices = [en_bos] + subword_encoder_en.encode(en_t.numpy()) + [en_bos + 1]
    # 同理，不过是使用中文字典的最后一个索引 + 1
    en_bos = subword_encoder_zh.vocab_size
    zh_indices = [en_bos] + subword_encoder_zh.encode(zh_t.numpy()) + [en_bos + 1]
...
# 验证集
val_dataset = (
    val_examples.map(tf_encode).filter(filter_max_length).padded_batch(BATCH_SIZE, padded_shapes=([-1], [-1])))
