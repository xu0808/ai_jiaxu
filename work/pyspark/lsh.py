...
if __name__ == '__main__':
    ...
    df = spark.createDataFrame(array, ["id", "features"])
    ...
    model.transform(df).show(20, False)
    ...
    # 2个数据集
    model.approxSimilarityJoin(df, df.limit(6), 5, distCol="EuclideanDistance") \
        .select(col("datasetA.id").alias("a"),
                col("datasetB.id").alias("b"),
                col("EuclideanDistance").alias("d"))\
        .orderBy("a", desc("d")).show(20, False)
    ...
    # 从dfA中找到2个与key距离最近的点
    model.approxNearestNeighbors(df, key, 2).show(20, False)
